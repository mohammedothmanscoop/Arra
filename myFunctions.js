...
function newcreate(flat) {
    ...
    myflat.children[4].appendChild(radio);
    ...
    myflat.children[3].appendChild(checkbox);

    myflat.children[2].innerHTML = `${flat.sall}`;
    myflat.children[2].setAttribute("name", "sall");

    myflat.children[1].innerHTML = `${flat.det}`;
    myflat.children[1].setAttribute("name", "det");

    myflat.children[0].innerHTML = `${flat.city}`;
    myflat.children[0].setAttribute("name", "city");
...
supbtn.onclick = function() {
    ...
    if (!phonePattern.test(phone.value) && phone.value != "") {
        ...
        alert("يرجى إدخال رقم هاتف صالح مع رمز البلد (مثال: 09396207838)");
        return false;
    }
